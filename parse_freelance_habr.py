...
def freelance_habr(page_limit, posts_limit):
    ...
    while True:
        ...
        try:
            first_injection = soup.find_all("div", {"class": "main-content"})[0]
            first_injection = first_injection.find_all("ul", {"class": "content-list content-list_tasks"})[0]
        except Exception as e:
            write_error(e, "Ошибка тега first_injection на странице " + str(page))
            break

        second_injection = first_injection.find_all("li", {"class": "content-list__item"})
        ...
        for post in second_injection:
            try:
                try:
                    title = post.find_all("article", {"class": "task task_list"})[0]
                    title = title.find_all("div", {"class": "task__column_desc"})[0]
                    title = title.find_all("header", {"class": "task__header"})[0]
                    title = title.find_all("div", {"class": "task__title"})[0]
                ...
                try:
                    price = post.find_all("article", {"class": "task task_list"})[0]
                    price = price.find_all("aside", {"class": "task__column_price"})[0]
                    price = price.find_all("div", {"class": "task__price"})[0]
                    if len(price.find_all("span", {"class": "count"})) == 1:
                        price = price.find_all("span", {"class": "count"})[0]
                        price = price.text.lstrip().rstrip()
                    elif len(price.find_all("span", {"class": "negotiated_price"})) == 1:
                        price = price.find_all("span", {"class": "negotiated_price"})[0]
                        price = price.text.lstrip().rstrip()
                except Exception:
                    raise ValueError("wrong tag of price")

                try:
                    link = post.find_all("article", {"class": "task task_list"})[0]
                    link = link.find_all("div", {"class": "task__column_desc"})[0]
                    link = link.find_all("header", {"class": "task__header"})[0]
                    link = link.find_all("div", {"class": "task__title"})[0]
                ...
                try:
                    date = post.find_all("article", {"class": "task task_list"})[0]
                    date = date.find_all("div", {"class": "task__column_desc"})[0]
                    date = date.find_all("header", {"class": "task__header"})[0]
                    date = date.find_all("div", {"class": "task__params params"})[0]
                    date = date.find_all("span", {"class": "params__published-at icon_task_publish_at"})[0]
